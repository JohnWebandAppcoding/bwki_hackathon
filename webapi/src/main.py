...
env = environment('40.468254,-86.980963', '48.445283,-84.948429')
...
map_wide = haversine(upper_left, upper_right) if haversine(upper_left, upper_right) > haversine(lower_left,
                                                                                                lower_right) else haversine(
...
if load_model == True:
    ...
    for current1, current2, action, reward, next1, next2, ds in zip(buffercurrents1, buffercurrents2, bufferactions,
                                                                    bufferrewards, buffernexts1, buffernexts2,
                                                                    bufferds):
        if ds == 'True':
            boo = True
        if ds == 'False':
            boo = False
        replay_buffer.buffer.extend(np.reshape(np.array(
            [[float(current1), float(current2)], int(action), float(reward), [float(next1), float(next2)], boo]),
                                               [1, 5]))

print("load buffer success............................")
print(replay_buffer.buffer)
print("------------Save training parameters--------------")
parameter = [env.map_bound, map_height, map_wide, step_length, wide_grid_num, height_grid_num, total_point,
             max_train_step, pre_train_step, learning_rate, step_rewardG, chargenumG, SOCG, timeG]
df_1 = pd.DataFrame([parameter])
df_1.to_csv("./ev/train_para.csv",
            header=["google map_boundary", "map_height(km)", "map_wide(km)", "step_length(m)", "wide_grid_num",
                    "height_grid_num", "total_point", "max_train_step in episode", "pre_train_step", "learning_rate",
                    "Google r", "Google charge num", "Google SOC", "Google time"])
...
with tf.Session() as sess:
    ...
    for episode in range(train_num):  # num of episode
        ...
        while (in_ep_step <= max_step):  # Max step in one episode
            ...
            if (total_step > pre_train and env.status_dir_check != 'OVER_QUERY_LIMIT' and len(
                    replay_buffer.buffer) > batch_num) or (
                    load_model == True and len(replay_buffer.buffer) > batch_num):  # start updating model
                if e > low_prob:
                    e -= slope
                ...
                # print(error)
                loss = sess.run(Qnet.loss, feed_dict={Qnet.input: np.vstack(ex_batch[:, 0]), Qnet.target_y: y,
                                                      Qnet.a: ex_batch[:, 1]})  # We use Qnet loss to update Qnet
                in_ep_loss = in_ep_loss + loss
                if total_step % 10 == 0:
                    loss_history.append(loss)
                # print(loss)
                # update model
                _ = sess.run(Qnet.update, feed_dict={Qnet.input: np.vstack(ex_batch[:, 0]), Qnet.target_y: y,
                                                     Qnet.a: ex_batch[:, 1]})  # input is s not s1
                istrain = 1  # try
            ...
            if d == True:
                ...
                real_reward = episode_reward + 0.1 * (
                            in_ep_step - env.unreach_position_num) - 1  # We don't penalize transition for real_reward(compare with google route)
                ...
                history = [episode + 1, in_ep_step, time, episode_reward, real_reward, real_r_nofail, charge_num, SOC,
                           env.unreach_position_num, d, random_a / (random_a + network_a), avg_loss, overQ_num,
                           loss_history, step_buffer]
                if episode == 0:
                    df = pd.DataFrame([history])
                    df.to_csv("./ev/result.csv",
                              header=["episode", "step", "time", "reward", "reward_notrain", "reward_nofail",
                                      "charge_num", "SOC", "unreach_position", "Reach", "Random_a", "Avg_loss",
                                      "overQuery_num", "Loss history", "Step history"])
                    tt = tt + 1
                elif episode > 0:
            ...
            if d == False and in_ep_step == max_step:
                ...
                time = env.time
                real_reward = episode_reward + 0.1 * (
                            in_ep_step - env.unreach_position_num)  # We don't penalize transition for real_reward(compare with google route) and we don't count the goal
                real_r_nofail = real_reward + env.unreach_position_num
                history = [episode + 1, in_ep_step, time, episode_reward, real_reward, real_r_nofail, charge_num, SOC,
                           env.unreach_position_num, d, random_a / (random_a + network_a), avg_loss, overQ_num,
                           loss_history, step_buffer]
