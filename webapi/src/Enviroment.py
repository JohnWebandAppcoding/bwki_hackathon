...
class environment():
    ...
    def geocoding_api(self, address):  # 2 output: status, position
        ...
        geocode_url = geocode_api + address + ".json?access_token=pk.eyJ1IjoicmVtZ2FsbGV1IiwiYSI6ImNraGhobGFnOTBodGEycW8xNDg2dzN3MnkifQ.IDObdcBtBfWNm-DVxeIZZw"
        ...
        if self.geocode_json_status == 'OK':
            latt = 0
            lngg = 0
            geocode_data_results = geocode_json['query']
            latt = geocode_data_results[0]
            lngg = geocode_data_results[1]
            self.latt = latt
            self.lngg = lngg
    ...
    def elevation_api(self, location):  # 2 output: elevation, resolution
        ...
        elevation_api = 'https://api.mapbox.com/v4/mapbox.mapbox-terrain-v2/tilequery/'
        elevation_url = elevation_api + location + ".json?&access_token=pk.eyJ1IjoicmVtZ2FsbGV1IiwiYSI6ImNraGhobGFnOTBodGEycW8xNDg2dzN3MnkifQ.IDObdcBtBfWNm-DVxeIZZw"
        ...
        if self.elevation_json_status == 'OK':
            elevation_data_features = elevation_json['features'][0]
            elevation_data_properties = elevation_data_features['properties']
            self.elevation_data_results_elevation = elevation_data_properties['ele']
        else:
    ...
    def directions_api(self, origin, destination):  # 5 output: status, steps, bound
        ...
        (lat_org, lng_org) = origin[0],origin[1]
        (lat_dst, lng_dst) = destination[0],destination[1]
        directions_api = 'https://api.mapbox.com/directions/v5/mapbox/driving-traffic/'
        directions_url = directions_api + lng_org + ',' + lat_org + ';' + lng_dst + ',' + lat_dst + "?geometries=geojson&access_token=pk.eyJ1IjoicmVtZ2FsbGV1IiwiYSI6ImNraGhobGFnOTBodGEycW8xNDg2dzN3MnkifQ.IDObdcBtBfWNm-DVxeIZZw"
        # directions_json = requests.get(directions_url, timeout=10).json()
        s = requests.Session()
        directions_json = s.get(directions_url).json()
        self.directions_json_status = 'OK'
        directions_data_routes = directions_json['routes']
        if self.directions_json_status == 'OK':
            directions_data_routes_bounds = directions_data_routes[0]['geometry']  # all are 'southwest', 'northeast'
            self.directions_data_routes_legs_steps = directions_data_routes_bounds['coordinates']
            self.directions_data_routes_bounds_dur = directions_data_routes[0]['legs'][0]['duration']
            self.len = len(self.directions_data_routes_legs_steps)
            print(self.len)
            for i in range(len):
            ...
            if lng_org > lng_dst:
                ...
                self.east = lng_dst
                self.west = lng_org
            self.bound = {'north': self.north, 'east': self.east, 'south': self.south,
                     'west': self.west}  # value of lat/lng
        else:
            # directions_data_routes_bounds = 'N/A'
            # directions_data_routes_legs = 'N/A'
            directions_data_routes_legs_steps = 'N/A'
    ...
    def make_map(self):
        ...
        while direction_status != 'OK':
            direction_status, direction_step, self.map_bound = self.directions_api(origin_position,
    ...
    def step(self, action):  # output:
        ...
        if status != 'OK' or self.next_position[0] > self.map_bound['north'] or self.next_position[0] < self.map_bound[
            'south'] or self.next_position[1] > self.map_bound['east'] or self.next_position[1] < self.map_bound[
            'west']:
            ...
            if abs(self.next_position[0] - self.end_position[0]) < self.stride_height and abs(
                    self.next_position[1] - self.end_position[1]) < self.stride_wide:
    ...
    def origine_map_reward(self):  # to get the step_reward, chargenum, SOC, time which the route google provided
        ...
        for i in range(len(leg)):
            print(leg)
            for j in range(2):
                start = (leg[i][j], leg[i][j])
                end = (leg[i+1][j], leg[i+1][j])
